...
namespace 
{
  void setDefaultModelPath(Classifier::Device device, std::string& out_model_xml, std::string& out_trained_file)
  {
    ...
    {
      std::cout << "Unknown device id " << static_cast<int>(device) << std::endl;
      throw std::runtime_error("Unknown device id " + std::to_string(static_cast<int>(device)));
    }
  }
}
                                       ...
                                       int batch_size)
{
 // --------------------Load IR Generated by ModelOptimizer (.xml and .bin files)--------------------------------------
  ...
  std::string model(model_file), weights(weights_file);
  if(model.empty() || weights.empty())
    setDefaultModelPath(device, model, weights);
  network_ = core.ReadNetwork(model, weights);
